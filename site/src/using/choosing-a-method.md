...
| Excitation energies | BSE-TDA on G0W0@HF; or CIS (TDA on an HF reference, exact within CIS) | CLI `bse-tda`, `tda`; Python `run_bse_tda`, `run_tddft` | BSE-TDA Smoke; TDA/TDDFT Spike (DFT references omit f<sub>xc</sub>) | — | `water-bse-tda.toml`, `water-tda.toml` |
