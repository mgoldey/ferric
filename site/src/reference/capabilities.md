...
| `gw` | [RPA/GW](../methods/rpa-gw.md) | RHF, or RKS via `[rpa] xc`; UHF/UKS when multiplicity > 1 (energy only) | ✓ (QP energies) | — | — | `run_gw`, `run_u_gw` | `water-g0w0-pbe.toml`, `oh-ugw.toml` | Smoke | Matches PySCF `gw_ac`/`ugw_ac` to ≤1.3e-7 Ha (open shell ≤7e-6 Ha) only at matched settings (`[rpa] n_quad = 100`, `trunc_thresh = 0`); the CLI defaults are coarser (the 20-point grid moves the H2O G0W0@PBE HOMO by 13 meV) and truncation is not validated. See [What is validated](./validation.md). |
