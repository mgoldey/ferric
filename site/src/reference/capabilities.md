...
| Constrained DFT | `run_cdft`, `CdftConstraint` | UHF, or UKS when `functional` is set (UKS is smoke-level). Fragment `target` is a Becke electron population (`"charge"`: Nα + Nβ; `"spin"`: Nα − Nβ), not a net charge. Raises if the λ loop does not converge. Not graded; see [Constrained DFT](../methods/cdft.md). |
