...
| RHF and UHF nuclear gradients with def2 ECPs: full gradient, the ECP term alone, translation invariance, 5-point finite difference of ferric's own energy | HI, CH3I / def2-SVP, def2-TZVP; SnH4 / def2-TZVP; CH2I (UHF doublet) / def2-SVP, def2-TZVP; HBr (all-electron control) | PySCF analytic gradient with its ECP term separated (checked against a fixed-density finite difference), and ORCA `EnGrad` (`NoRI`), both fed ferric's basis and ECP | gradient ≤ 1.3e-7 Ha/Bohr vs PySCF and ≤ 2.5e-7 vs ORCA; the ECP term alone ≤ 1.3e-7; own 5-point FD ≤ 1.9e-7; energy ≤ 2.9e-8 Ha | 5e-7 Ha/Bohr (PySCF), 1e-6 (ORCA), 6e-7 (FD) | [`validation_ecp_gradient.rs`](https://github.com/mgoldey/ferric/blob/main/crates/ferric-scf/tests/validation_ecp_gradient.rs), [`gen_ecp_gradient.py`](https://github.com/mgoldey/ferric/blob/main/scripts/validation/gen_ecp_gradient.py) |
