...
def rmsds_to_bound(smiles, symbols, frames, ref_symbols, ref_coords):
    ...
    for f in frames:
        al = align_to_reference(
            smiles, symbols, f, DANUGLIPRON_SMILES, ref_symbols, ref_coords
        )
        if al.ok:
...
def main() -> int:
    ...
    print(
        f"pocket: {pocket.n_charges} charges; target bar "
        f"{DOCKING_SUCCESS_RMSD_ANGSTROM:.1f} A",
        flush=True,
    )
    ...
    for k, coords in enumerate(emb.conformers):
        ...
        for tag, pc in (("in_field", near), ("vacuum", None)):
            ...
            if not run.ok:
                ...
                continue
            rs = rmsds_to_bound(
                ana.scoring_smiles, emb.symbols, frames, ref_symbols, ref_coords
            )
            if not rs:
            ...
            rec[tag] = {
                ...
                "mean": st.mean(rs),
                "worst": max(rs),
                "improvement_vs_start": start_rmsd - min(rs),
                "wall_seconds": dt,
                "all_rmsds": rs,
            }
            print(
                f"  {tag:9s} {len(rs):2d} frames  best {min(rs):5.2f}  "
    ...
    print("\n" + "=" * 74)
    fielded = [
        r["in_field"] for r in results["starts"] if "best" in r.get("in_field", {})
    ]
    ...
    if fielded:
        ...
        results["reached_docking_bar"] = bool(
            best_overall < DOCKING_SUCCESS_RMSD_ANGSTROM
        )
        print(
            f"best in-field frame:        {best_overall:.2f} A "
        ...
        if vac:
            ...
            print(
                f"  field advantage:          {vb - best_overall:+.2f} A "
