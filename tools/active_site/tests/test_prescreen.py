...
def test_prescreen_pose_charge_count_mismatch_raises():
    pocket = PocketCharges(
        charges=[(1.0, 10.0, 10.0, 10.0)],
        source_pdb=Path("fake.pdb"),
        ff="AMBER",
    )
    embedded = embed_ligand_from_coords(
        WATER_SYMBOLS, WATER_COORDS, pocket=pocket, basis="sto-3g"
    )
...
def test_batch_prescreen_ranks_ascending_by_score():
    ...
    ranked = batch_prescreen(
        pocket,
        [METHANE_XYZ, WATER_XYZ],
        charge_source=_formal_charges_by_natoms,
        basis="sto-3g",
    )
    assert len(ranked) == 2
...
def test_batch_prescreen_isolates_per_pose_failures():
    ...
    ranked = batch_prescreen(
        ...
        charge_source=flaky_charge_source,
        basis="sto-3g",
    )
    assert len(ranked) == 2
...
def test_batch_prescreen_missing_file_is_a_per_pose_failure_not_a_crash():
    ...
    ranked = batch_prescreen(
        pocket,
        [WATER_XYZ, "testdata/molecules/does_not_exist.xyz"],
        charge_source=_formal_charges_by_natoms,
        basis="sto-3g",
    )
    assert len(ranked) == 2
