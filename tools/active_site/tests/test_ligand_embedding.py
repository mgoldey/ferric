...
def test_embed_ligand_with_pocket_filters_overlap():
    ...
    pocket = PocketCharges(
        charges=[
            (1.0, 0.0, 0.0, 0.117790 * 1.8897261245650618),
    ...
    embedded = embed_ligand(
        WATER_XYZ, pocket=pocket, basis="sto-3g", overlap_cutoff_angstrom=0.1
    )
...
def test_embed_ligand_from_coords_matches_file():
    ...
    from_coords = embed_ligand_from_coords(
        ["O", "H", "H"], WATER_COORDS, basis="sto-3g"
    )
