...
def phi_terfc(rho, S, D, r0):
    ...
    def phi_cosh(tau):
        ...
        g = 2 * A_CONST * tau
        return mp.mpf("0.5") * (
            phi_gauss_linear(c, g, rho, S, D) + phi_gauss_linear(c, -g, rho, S, D)
        )
...
if __name__ == "__main__":
    ...
    print("=" * 72)
    print(
        "PROOF 3 (NEGATIVE)  shipped G_{0,n}(S,s_bp) cannot fit Phi_h (blueprint FALSE)"
    )
    print("=" * 72)
    ...
    def _bf(x, dimk, dimi):
        pp = poisson_pmf(x, dimi)
        g = [[mp.mpf(0)] * dimi for _ in range(dimk)]
        g[1] = list(pp)
        tot = mp.mpf(0)
        for i in range(dimi):
            tot += g[1][i]
    ...
    def G0n_shipped(n, S, s):
        df = df_precompute(DIMI)
        gS = _bf(S, 2, DIMI)
        gs = _bf(s, max(n + 1, 2), DIMI)
        return sum(df[i] * gS[1][i] * gs[n][i] for i in range(DIMI))

    w = 1 / (r0 * mp.sqrt(2))
    s_bp = rho * w * w / (rho + w * w) * r0 * r0
    ...
    for N in [4, 8, 11]:
        ...
        print(
            "  N=%2d  fit rms=%.3e  |c|_max=%.2e   (does NOT converge)"
