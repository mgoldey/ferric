...
print(
    "system      nat    diam | " + " ".join(f"r={r:<5.1f}" for r in RADII), flush=True
)
rows = []
for name in ("alkane_2", "alkane_4", "alkane_6", "alkane_8", "alkane_10"):
    ...
    try:
        dense = ferric.run_laplace_sos_mp2(
            m, bs, aux, c_os=1.0, formulation="ao", memory_budget_gb=8.0
        ).e_os
    except Exception as e:
        print(f"{name:10} {nat:>4} {d:>7.1f} | DENSE FAILED: {str(e)[:50]}", flush=True)
        continue
    cells = []
    r_chem = None
    for r in RADII:
        try:
            e = ferric.run_laplace_sos_mp2(
                m,
                bs,
                aux,
                c_os=1.0,
                formulation="ao-sparse",
                domain_cutoff_bohr=r,
                memory_budget_gb=8.0,
            ).e_os
            rel = abs(e - dense) / abs(dense)
            cells.append(f"{rel:7.1e}")
            if r_chem is None and abs(e - dense) < 1.6e-3:
                r_chem = r
        except Exception as ex:
            cells.append("  ERR  ")
    print(
        f"{name:10} {nat:>4} {d:>7.1f} | " + " ".join(cells) + f"   r(chem)={r_chem}",
        flush=True,
    )
...
for name, d, rc in rows:
    print(
        f"{name:10} {d:>8.1f} {str(rc):>9} {(f'{rc / d:.2f}' if rc else 'n/a'):>8}",
        flush=True,
    )
