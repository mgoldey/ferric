...
print(
    f"{'system':11}{'nat':>4}{'nbas':>6}{'diam':>7}"
    f"{'f>1e-6':>9}{'f>1e-8':>9}{'nnz6':>10}{'pairs/atom':>12}",
    flush=True,
)
rows = []
for n in ("alkane_4", "alkane_8", "alkane_12", "alkane_16", "alkane_18", "alkane_20"):
    ...
    nat = len(c)
    d = max(math.dist(x, y) for x in c for y in c) * 1.8897259886
    m = ferric.Molecule.from_xyz(p)
    try:
        r = ferric.run_rhf(m, bs)
    except Exception as e:
        print(f"{n:11}{nat:>4} RHF FAIL {str(e)[:38]}", flush=True)
        continue
    a = np.abs(np.asarray(r.density()))
    nb = a.shape[0]
    n6 = int((a > 1e-6).sum())
    f6 = n6 / a.size
    f8 = (a > 1e-8).sum() / a.size
    print(
        f"{n:11}{nat:>4}{nb:>6}{d:>7.1f}{f6:>9.3f}{f8:>9.3f}{n6:>10}{n6 / nat:>12.1f}",
        flush=True,
    )
    rows.append((nat, nb, f6, n6))
if len(rows) >= 3:
    nb = np.array([r[1] for r in rows], float)
    n6 = np.array([r[3] for r in rows], float)
    print(
        "\nLOCAL exponent between successive systems (the informative number):",
        flush=True,
    )
    ...
    print(
        f"\nglobal fit  nnz ~ nbas^{g:.2f}  <-- averages in pre-onset points, MISLEADING",
        flush=True,
    )
    if len(nb) >= 3:
    ...
    print(
        "\n2.0 = fully dense, 1.0 = linear scaling. Falling local exponent = sparsity onset.",
        flush=True,
    )
