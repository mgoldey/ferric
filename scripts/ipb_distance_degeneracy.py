...
def scan(name, basis, atom_grid):
    ...
    return dict(
        ...
        nsh=mol.nbas,
        nbf=mol.nao,
        nbatch=math.ceil(coords.shape[0] / BATCH_POINTS),
        tot=tot,
        deg_ipb=di / tot,
        deg_fe=df / tot,
        deg_both=dboth / tot,
        gflat50=np.median(gf_),
        gflat90=np.percentile(gf_, 90),
        gflatmax=gf_.max(),
        gfe50=np.median(gfe),
        gfe90=np.percentile(gfe, 90),
...
def main():
    ...
    ap.add_argument(
        "--systems", default="alkane_1,alkane_2,alkane_4,alkane_8,alkane_12,alkane_16"
    )
    ...
    print(
        f"{'system':<11}{'diam':>7}{'nsh':>5}{'nbat':>6}{'decisions':>11}"
    ...
    for name in args.systems.split(","):
        ...
        print(
            f"{r['name']:<11}{r['diam']:7.2f}{r['nsh']:5d}{r['nbatch']:6d}"
