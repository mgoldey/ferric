...
data = [
    ("h2o", 5.901, 27.05, 0.1759),
    ("ch4", 10.443, 34.40, 0.1255),
    ("nh3", 8.529, 32.43, 0.1035),
    ("co", 8.348, 46.35, 0.3651),
    ("n2", 7.886, 44.80, 0.4640),
    ("co2", 11.315, 59.20, 0.3712),
    ("c2h4", 16.899, 61.08, 0.2267),
]
a = np.array([d[1] for d in data])
V = np.array([d[2] for d in data])
wopt = np.array([d[3] for d in data])
aoV = a / V
y = (4 * np.pi / 3) * aoV
eps = (1 + 2 * y) / (1 - y)
