...
def analyze_basis(basis_key, basis_label):
    ...
    k_candidates = {
        m: fit_k(ref, rpa, m)
        for m in ("median_ratio", "mean_ratio", "ratio_of_means", "least_squares")
    }
    ...
    for m, k in k_candidates.items():
        ...
        print(
            f"  {m:16s} k={k:.4f}  in-sample MAE={err.mean():.2f}%  "
    ...
    print(
        f"LOO-CV per-fold k range: [{loo_ks.min():.4f}, {loo_ks.max():.4f}], "
    ...
    print(
        f"  Win rate: rescaled-RPA closer on {rpa_wins}/{n}, "
...
def main():
    ...
    print(
        f"Applying aTZ's k={k_tz:.4f} to aDZ data (WRONG basis transfer): "
