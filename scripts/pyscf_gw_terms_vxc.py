...
# vxc in MO basis
dm = mf.make_rdm1()
vxc_ao = mf.get_veff(mol, dm) - mf.get_j(
    mol, dm
)  # v_xc = veff - J (for pure GGA, no HF exch)
...
print(
    f"implied (Σx-vxc) consistency: ε_KS + Σx - vxc = {(eps_ks + (vk[homo] * HA if vk is not None else 0) - vxc_mo[homo] * HA):8.4f}"
)
