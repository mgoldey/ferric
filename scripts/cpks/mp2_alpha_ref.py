...
print(
    "field SCF + MP2 P built. Poo diag:",
    np.round(np.diag(Poo), 5),
    "Pvv diag:",
    np.round(np.diag(Pvv), 5),
)
        ...
        for k in range(nocc):
            ...
            for j in range(nocc):
                for a in range(nvir):
                    for b in range(nvir):
                        g += t[k, a, j, b] * (
                            2 * Imo[nocc + c, nocc + a, j, nocc + b]
                            - Imo[nocc + c, nocc + b, j, nocc + a]
                        )
            for i in range(nocc):
                for a in range(nvir):
                    for b in range(nvir):
                        g += t[i, a, k, b] * (
                            2 * Imo[i, nocc + a, nocc + c, nocc + b]
                            - Imo[i, nocc + b, nocc + c, nocc + a]
                        )
            for i in range(nocc):
                for j in range(nocc):
                    for b in range(nvir):
                        g -= t[i, c, j, b] * (
                            2 * Imo[i, k, j, nocc + b] - Imo[i, nocc + b, j, k]
                        )
            for i in range(nocc):
                for j in range(nocc):
                    for a in range(nvir):
                        g -= t[i, a, j, c] * (
                            2 * Imo[i, nocc + a, j, k] - Imo[i, k, j, nocc + a]
                        )
            L[c, k] = g
    return L
...
def dipole_expect(Ffield):
    ...
    return np.sum(
        D * rmo
    )  # Tr[D r] in field-MO basis = <r> (r transformed consistently)
...
def analytic_alpha():
    ...
    dPoo = -(
        np.einsum("iakb,jakb->ij", dt, 2 * t - t.transpose(0, 3, 2, 1))
        + np.einsum("iakb,jakb->ij", t, 2 * dt - dt.transpose(0, 3, 2, 1))
    )
    dPvv = np.einsum("iajc,ibjc->ab", dt, 2 * t - t.transpose(2, 1, 0, 3)) + np.einsum(
        "iajc,ibjc->ab", t, 2 * dt - dt.transpose(2, 1, 0, 3)
    )
    # ∂L = directional deriv of lagrangian along (Imo→dImo, t→dt)
    epsd = 1e-6
    Lp = lagrangian(
        Imo + epsd * dImo, t + epsd * dt, e
    )  # note: e fixed; ∂ε enters via denom in t already
    ...
    dAz0 = (
        Az(Imo + epsd * dImo, e + epsd * de) - Az(Imo - epsd * dImo, e - epsd * de)
    ) / (2 * epsd)
    ...
    for a in range(nvir):
        for i in range(nocc):
            dDrel[nocc + a, i] += 2 * U[a, i] + dz[a, i]
            dDrel[i, nocc + a] += 2 * U[a, i] + dz[a, i]
    dDrel[O, O] += dPoo
    dDrel[Vv, Vv] += dPvv
...
for a in range(nvir):
    for i in range(nocc):
        dDrel[nocc + a, i] += 2 * U_fd[a, i] + dz_fd[a, i]
        dDrel[i, nocc + a] += 2 * U_fd[a, i] + dz_fd[a, i]
dDrel[O, O] += dPoo_fd
dDrel[Vv, Vv] += dPvv_fd
...
print(
    "SUM                      =", -np.sum(dDrel * r) + term_Dr, " (oracle", oracle, ")"
)

print("\n=== KEY FINDINGS (traceable clean-room) ===")
print(
    "True ∂z (phase-matched FD)  ‖·‖ = %.2e  <-- TINY (MP2 barely shifts α)"
    % np.max(np.abs(dz_fd))
)
print(
    "Analytic ∂z (current Rust formula) ‖·‖ = %.2e  <-- ~%.0fx TOO LARGE"
    % (np.max(np.abs(dz_an)), np.max(np.abs(dz_an)) / max(np.max(np.abs(dz_fd)), 1e-30))
)
...
print(
    "‖dz_fd - z_rot‖   =",
    np.max(np.abs(dz_fd - z_rot)),
    "  <-- if small, dz_fd is MOSTLY just z0 rotating",
)
    ...
    for a in range(nvir):
        for j in range(nocc):
            for b in range(nvir):
                d = e[i] + e[j] - e[nocc + a] - e[nocc + b]
                dt[i, a, j, b] = (
                    dImo[i, nocc + a, j, nocc + b]
                    - t[i, a, j, b] * (de[i] + de[j] - de[nocc + a] - de[nocc + b])
                ) / d
ed = 1e-6
dL = (
    lagrangian(Imo + ed * dImo, t + ed * dt, e)
    - lagrangian(Imo - ed * dImo, t - ed * dt, e)
) / (2 * ed)
...
print(
    "∂z (with ∂A, no separate ∂Δε) ‖·‖ =",
    np.max(np.abs(dz_full)),
    " vs dz_fd",
    np.max(np.abs(dz_fd)),
)
...
print(
    "  ‖rhs_full‖ =",
    np.max(np.abs(rhs_full)),
    " (∂L=84, so cancellation left:",
    np.max(np.abs(rhs_full)),
    ")",
)
...
print(
    ...
    np.max(np.abs(dImo)),
    " ‖dImo_fd‖=",
    np.max(np.abs(dImo_fd)),
    " ‖Δ‖=",
    np.max(np.abs(dImo - dImo_fd)),
)
...
dImo_true = (
    np.einsum("xp,xqrs->pqrs", Th_true, Imo)
    + np.einsum("xq,pxrs->pqrs", Th_true, Imo)
    + np.einsum("xr,pqxs->pqrs", Th_true, Imo)
    + np.einsum("xs,pqrx->pqrs", Th_true, Imo)
)
...
for s in [1.0, 0.5, 2.0]:
    ...
    print(
        f"  U(s={s}): ‖U-Θ_true‖={np.max(np.abs(Us - Th_true[nocc:, :nocc])):.5f}  HFα(×-4)={-4 * np.sum(Us * rvo):.5f}"
    )
...
print(
    "‖(Δε+A)Utrue + rvo‖ =",
    np.max(np.abs(res_full)),
    " (small ⇒ full A, RHS -rvo correct)",
)
res_half = (Adiag @ Utrue.reshape(-1) + 0.5 * (Acoup @ Utrue.reshape(-1))).reshape(
    nvir, nocc
) + rvo
...
print(
    "‖M·Utrue + rvo‖ =",
    np.max(np.abs((Mh @ Utrue.reshape(-1)).reshape(nvir, nocc) + rvo)),
)
