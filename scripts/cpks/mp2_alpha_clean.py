...
def t2_amp(Imo, e):
    ...
    for i in range(nocc):
        for a in range(nvir):
            for j in range(nocc):
                for b in range(nvir):
                    t[i, a, j, b] = Imo[i, nocc + a, j, nocc + b] / (
    ...
    for i in range(nocc):
        for a in range(nvir):
            for j in range(nocc):
                for b in range(nvir):
                    s += t[i, a, j, b] * (
                        2 * Imo[i, nocc + a, j, nocc + b]
                        - Imo[i, nocc + b, j, nocc + a]
                    )
    return s
...
def P_blocks(Imo, e):
    ...
    t = t2_amp(Imo, e)
    tij = t.transpose(0, 2, 1, 3)  # [i,j,a,b]
    P_oo = -(
        2 * np.einsum("ikab,jkab->ij", tij, tij) - np.einsum("ikab,jkba->ij", tij, tij)
    )
    P_vv = 2 * np.einsum("ijca,ijcb->ab", tij, tij) - np.einsum(
...
def dipole_mo(D_mo, Cmo):
    ...
    D_ao = Cmo @ D_mo @ Cmo.T
    return np.array(
        [-np.einsum("pq,pq->", dip_ao[d], D_ao) + nucl[d] for d in range(3)]
    )
...
def oracle_alpha_energy_hessian(h=1e-3):
    ...
    for q in range(3):
        # diagonal
        a[q, q] = (
            -(e_mp2_total_field(h, q) - 2 * E00 + e_mp2_total_field(-h, q)) / h**2
        )  # α=-d²E/dF²
    ...
    for q in range(3):
        for p in range(q + 1, 3):
            ...
            def E2(fp, fq):
                m = scf.RHF(mol)
                m.get_hcore = lambda *a: hcore_ao - fp * dip_ao[p] - fq * dip_ao[q]
                m.conv_tol = 1e-12
                m.kernel()
                return m.e_tot + mp.MP2(m).run().e_corr
                # noqa

            mixed = -(E2(h, h) - E2(h, -h) - E2(-h, h) + E2(-h, -h)) / (
                4 * h**2
            )  # -d²E/dFpdFq
            a[p, q] = a[q, p] = mixed
    return a
...
def oracle_relaxed_dipole_FF():
    ...
    mu = np.zeros(3)
    for ax in range(3):
        mu[ax] = +(e_mp2_total_field(h, ax) - e_mp2_total_field(-h, ax)) / (
            2 * h
        )  # μ=+dE/dF (hcore-F·r)
    return mu
...
def alpha_B(h=1e-3):
    a = np.zeros((3, 3))
    for q in range(3):
        mp_ = relaxed_dipole_in_field(h, q)
        mm_ = relaxed_dipole_in_field(-h, q)
...
def cphf_U(axis):
    rvo = np.array(
        [[r_mo0[axis, nocc + a, i] for i in range(nocc)] for a in range(nvir)]
    )
...
for a in range(nvir):
    for i in range(nocc):
        ...
        Th[i, nocc + a] = -U[a, i]
dImo = (
    np.einsum("xp,xqrs->pqrs", Th, Imo0)
    + np.einsum("xq,pxrs->pqrs", Th, Imo0)
    + np.einsum("xr,pqxs->pqrs", Th, Imo0)
    + np.einsum("xs,pqrx->pqrs", Th, Imo0)
)
...
ed = 1e-5
dD_an = (
    relaxed_dm(Imo0 + ed * dImo, e0 + ed * deps)
    - relaxed_dm(Imo0 - ed * dImo, e0 - ed * deps)
) / (2 * ed)
for a in range(nvir):
    for i in range(nocc):
        dD_an[nocc + a, i] += 2 * U[a, i]
        dD_an[i, nocc + a] += 2 * U[a, i]
print("  α_yy from analytic ∂D:", -np.sum(dD_an * r_mo0[1]))
print("  ‖dD_an - dD_fd‖ =", np.max(np.abs(dD_an - dD_fd)))
print(
    "  block diffs: oo=%.4f vv=%.4f ov=%.4f"
    % (
        np.max(np.abs((dD_an - dD_fd)[o, o])),
        np.max(np.abs((dD_an - dD_fd)[v, v])),
        np.max(np.abs((dD_an - dD_fd)[v, o])),
    )
)
    ...
    for q in range(3):
        ...
        for aa in range(nvir):
            for ii in range(nocc):
                Th[nocc + aa, ii] = U[aa, ii]
                Th[ii, nocc + aa] = -U[aa, ii]
        dImo = (
            np.einsum("xp,xqrs->pqrs", Th, Imo0)
            + np.einsum("xq,pxrs->pqrs", Th, Imo0)
            + np.einsum("xr,pqxs->pqrs", Th, Imo0)
        ...
        for aa in range(nvir):
            for ii in range(nocc):
                dDscf[nocc + aa, ii] += 2 * U[aa, ii]
                dDscf[ii, nocc + aa] += 2 * U[aa, ii]
        ...
        ed = 1e-5
        dD = (relaxed_dm(Imo0 + ed * dImo, e0) - relaxed_dm(Imo0 - ed * dImo, e0)) / (
            2 * ed
        )
        for aa in range(nvir):
            for ii in range(nocc):
                dD[nocc + aa, ii] += 2 * U[aa, ii]
                dD[ii, nocc + aa] += 2 * U[aa, ii]
        for p in range(3):
...
hh = 1e-4
ep, _ = scf_in_field(hh, q)
em, _ = scf_in_field(-hh, q)
deps_fd = (ep - em) / (2 * hh)
...
# ===========================================================================
def G_ao(dm_ao):
    J = np.einsum("pqrs,rs->pq", eri_ao, dm_ao)
    K = np.einsum("prqs,rs->pq", eri_ao, dm_ao)
    return 2 * J - K
...
for a in range(nvir):
    for i in range(nocc):
        dDscf_an_ao += (
            2
            * U[a, i]
            * (np.outer(Cvir[:, a], Cocc[:, i]) + np.outer(Cocc[:, i], Cvir[:, a]))
        )
