    ...
    for i in range(nocc):
        for a in range(nvir):
            for j in range(nocc):
                for b in range(nvir):
                    K = (
                        2 * Imatrix[i, nocc + a, j, nocc + b]
                        - Imatrix[i, nocc + b, j, nocc + a]
                    )
                    s += t[i, a, j, b] * K
    return s
...
def cphf_U(axis):
    rvo = np.array(
        [[r_mo[axis, nocc + a, i] for i in range(nocc)] for a in range(nvir)]
    )
...
for axis in range(3):
    ...
    print(
        f"  axis {axis}: FF={ff:+.5f}  -4ΣUr={-4 * np.sum(U * rvo):+.5f}  -2ΣUr={-2 * np.sum(U * rvo):+.5f}"
    )
        ...
        for k in range(nocc):
            ...
            for j in range(nocc):
                for a in range(nvir):
                    for b in range(nvir):
                        g += t[k, a, j, b] * (
                            2 * Imatrix[nocc + c, nocc + a, j, nocc + b]
                            - Imatrix[nocc + c, nocc + b, j, nocc + a]
                        )
            for i in range(nocc):
                for a in range(nvir):
                    for b in range(nvir):
                        g += t[i, a, k, b] * (
                            2 * Imatrix[i, nocc + a, nocc + c, nocc + b]
                            - Imatrix[i, nocc + b, nocc + c, nocc + a]
                        )
            for i in range(nocc):
                for j in range(nocc):
                    for b in range(nvir):
                        g -= t[i, c, j, b] * (
                            2 * Imatrix[i, k, j, nocc + b] - Imatrix[i, nocc + b, j, k]
                        )
            for i in range(nocc):
                for j in range(nocc):
                    for a in range(nvir):
                        g -= t[i, a, j, c] * (
                            2 * Imatrix[i, nocc + a, j, k] - Imatrix[i, k, j, nocc + a]
                        )
            L[c, k] = g
    return L
...
def relaxed_dm_mo(Imatrix, evec):
    ...
    z = np.linalg.solve(M, L.reshape(-1)).reshape(
        nvir, nocc
    )  # NB: M built from unpert. Imo
    D = np.zeros((nmo, nmo))
    for i in range(nocc):
        D[i, i] += 2.0
    D[O, O] += Poo
...
print(
    "oo block PySCF diag:",
    np.round(np.diag(dm1_mo)[O], 5),
    " mine:",
    np.round(np.diag(Dmine)[O], 5),
)
print(
    "vv block PySCF diag:",
    np.round(np.diag(dm1_mo)[Vv], 5),
    " mine:",
    np.round(np.diag(Dmine)[Vv], 5),
)
ov_ps = dm1_mo[Vv, O]
ov_me = Dmine[Vv, O]
...
print(
    "‖oo mine-pyscf‖=",
    np.max(np.abs((Dmine - dm1_mo)[O, O])),
    " ‖vv‖=",
    np.max(np.abs((Dmine - dm1_mo)[Vv, Vv])),
    " ‖ov‖=",
    np.max(np.abs((Dmine - dm1_mo)[Vv, O])),
)
...
doo = 2 * np.einsum("ikab,jkab->ij", t_ijab, t_ijab) - np.einsum(
    "ikab,jkba->ij", t_ijab, t_ijab
)
dvv = 2 * np.einsum("ijca,ijcb->ab", t_ijab, t_ijab) - np.einsum(
    "ijca,ijbc->ab", t_ijab, t_ijab
)
Poo_es = -doo
Pvv_es = dvv
...
print(
    "Pvv_einsum diag:",
    np.round(np.diag(Pvv_es), 5),
    " (PySCF dm1 vv diag was",
    np.round(np.diag(dm1_mo)[Vv], 5),
    ")",
)
print(
    "  → einsum Pvv vs pyscf-stored (doo+doo.T means stored = Pvv+Pvv.T):",
...
def pyscf_corr_dipole(axis, h=1e-4):
    def ecorr(F):
        m = scf.RHF(mol)
        m.get_hcore = lambda *a: hcore_ao - F * dip_ao[axis]
        m.conv_tol = 1e-12
        m.kernel()
...
t_iajb = t
Poo = -(
    2 * np.einsum("ikab,jkab->ij", t_ijab, t_ijab)
    - np.einsum("ikab,jkba->ij", t_ijab, t_ijab)
)
Pvv = 2 * np.einsum("ijca,ijcb->ab", t_ijab, t_ijab) - np.einsum(
    "ijca,ijbc->ab", t_ijab, t_ijab
)
...
for a in range(nvir):
    for i in range(nocc):
        Dcorr[nocc + a, i] += z[a, i]
        Dcorr[i, nocc + a] += z[a, i]
mu_corr_mine = (
    dipole_from_dm_ao(C @ Dcorr @ C.T) - nucl
)  # pure electronic correlation part
print("\n=== correlation dipole (relaxed-HF) mine vs pyscf ===")
print("  mine  =", mu_corr_mine)
print("  pyscf =", mu_corr_ps)
print(
    "  ratio z (mine/pyscf) on z-axis:",
    mu_corr_mine[2] / mu_corr_ps[2] if abs(mu_corr_ps[2]) > 1e-9 else "na",
)
...
print(
    "  pyscf UNRELAXED corr dipole_z =",
    (dipole_from_dm_ao(dm1_ao) - mu_hf)[2],
    " (= P-blocks only; mine P-only=",
    mu_P[2],
    ")",
)
...
for scale in [1.0, 2.0]:
    ...
    print(
        f"  RHS=L+{scale}·Gvo: corr dipole_z = {mz:.6f}  (target {mu_corr_ps[2]:.6f})"
    )
...
try:
    ...
    print(
        "  pyscf doo diag:",
        np.round(np.diag(doo_ps), 6),
        " mine -Poo? ",
        np.round(np.diag(-Poo), 6),
    )
    print(
        "  pyscf dvv diag:",
        np.round(np.diag(dvv_ps), 6),
        " mine Pvv:",
        np.round(np.diag(Pvv), 6),
    )
except Exception as ex:
...
print(
    f"  pure-z(s=1) dipole_z = {zonly:.6f}; L-scale to hit target = {target_z_dip / zonly:.4f}"
)
...
print(
    "  P+z_ps corr dipole_z =",
    mu_with_psz,
    " oracle =",
    mu_corr_ps[2],
    " match:",
    abs(mu_with_psz - mu_corr_ps[2]) < 1e-5,
)
...
for i in range(nocc):
    dm1_full[i, i] = 2.0
dm1_full[O, O] += Poo + Poo.T
dm1_full[Vv, Vv] += Pvv + Pvv.T
veff_vo = G_mo(dm1_full)[Vv, O]  # 2J-K of full dm1
veff_vo_P = G_mo(dmP)[
    Vv, O
]  # 2J-K of P-only (HF part gives the SCF Fock = diagonal, no vo)
...
print(
    "  ‖Pop·z_ps + Xvo_ps‖ =",
    np.max(np.abs(Pop(z_ps.reshape(-1)) + Xvo_ps)),
    " (0 ⇒ solves (Δε+f)z=-Xvo)",
)
...
src = (
    inspect.getsource(cphf.solve_nos1)
    if hasattr(cphf, "solve_nos1")
    else inspect.getsource(cphf.solve)
)
print(
    "  cphf.solve sign hint:",
    [
        l.strip()
        for l in src.split(chr(10))
        if "return" in l or "-" in l and "mo_energy" in l
    ][:3],
)
...
dm1_relaxed_ps[Vv, O] += z_ps
dm1_relaxed_ps[O, Vv] += z_ps.T
...
print(
    "  *** STATIC RELAXED DENSITY MATCHES PYSCF:",
    np.max(np.abs(mu_me - mu_ps)) < 1e-6,
    "***",
)
...
def static_relaxed_dm_validated(Imatrix, evec):
    ...
    Poo = -(
        2 * np.einsum("ikab,jkab->ij", tij, tij) - np.einsum("ikab,jkba->ij", tij, tij)
    )
    Pvv = 2 * np.einsum("ijca,ijcb->ab", tij, tij) - np.einsum(
        "ijca,ijbc->ab", tij, tij
    )
    dmP = np.zeros((nmo, nmo))
    dmP[O, O] = Poo + Poo.T
    dmP[Vv, Vv] = Pvv + Pvv.T
    ...
    J = np.einsum("pqrs,rs->pq", Imatrix, dmP)
    K = np.einsum("prqs,rs->pq", Imatrix, dmP)
    Gvo = (2 * J - K)[Vv, O]
    Xvo = L + Gvo
        ...
        for a in range(nvir):
            for i in range(nocc):
                for b in range(nvir):
                    for j in range(nocc):
                        Mm[a, i, b, j] = (
                            4 * Imatrix[nocc + a, i, nocc + b, j]
                            - Imatrix[nocc + a, nocc + b, i, j]
                            - Imatrix[nocc + a, j, nocc + b, i]
                        )
    ...
    for q in range(3):
        ...
        for a in range(nvir):
            for i in range(nocc):
                ...
                Th[i, nocc + a] = -U[a, i]
        dImo = (
            np.einsum("xp,xqrs->pqrs", Th, Imo)
            + np.einsum("xq,pxrs->pqrs", Th, Imo)
            + np.einsum("xr,pqxs->pqrs", Th, Imo)
            + np.einsum("xs,pqrx->pqrs", Th, Imo)
        )
        # ∂eps_p = ∂F_pp = (-r + G[∂D_scf])_pp ; ∂D_scf from U (2δ core response)
        dDscf = np.zeros((nmo, nmo))
        for a in range(nvir):
            for i in range(nocc):
                dDscf[nocc + a, i] += 2 * U[a, i]
                dDscf[i, nocc + a] += 2 * U[a, i]
        G = 2 * np.einsum("pqrs,rs->pq", Imo, dDscf) - np.einsum(
            "prqs,rs->pq", Imo, dDscf
        )
        ...
        for a in range(nvir):
            for i in range(nocc):
                dDrel[nocc + a, i] += 2 * U[a, i]
                dDrel[i, nocc + a] += 2 * U[a, i]
...
print(
    "  analytic diag:", np.round(np.diag(aa), 5), " iso", np.round(np.trace(aa) / 3, 5)
)
...
eo_p, Cp = Cmo_field(hh)
eo_m, Cm = Cmo_field(-hh)
...
dImo_fd = (Imo_p - Imo_m) / (2 * hh)
deps_fd = (eo_p - eo_m) / (2 * hh)
...
def emp2_from_I(Imatrix, evec):
    return e_mp2(Imatrix, evec)


ed = 1e-5
dE_an = (
    emp2_from_I(Imo + ed * dImo_an, e + ed * deps_an)
    - emp2_from_I(Imo - ed * dImo_an, e - ed * deps_an)
) / (2 * ed)
...
def emp2_field(F):
    m = scf.RHF(mol)
    m.get_hcore = lambda *a: hcore_ao - F * dip_ao[1]
    m.conv_tol = 1e-12
    m.kernel()
    return mp.MP2(m).run().e_corr
...
def Pvv_from_I(Imatrix, evec):
    t = t2_amp(Imatrix, evec)
    tij = t.transpose(0, 2, 1, 3)
    return 2 * np.einsum("ijca,ijcb->ab", tij, tij) - np.einsum(
        "ijca,ijbc->ab", tij, tij
    )
...
ed = 1e-5
dPvv_an = (
    Pvv_from_I(Imo + ed * dImo_an, e + ed * deps_an)
    - Pvv_from_I(Imo - ed * dImo_an, e - ed * deps_an)
) / (2 * ed)
# FD: P_vv in field-MO basis (phase-matched)
hh = 1e-4
Pvv_p = Pvv_from_I(Imo_p, eo_p)
Pvv_m = Pvv_from_I(Imo_m, eo_m)
...
inv_an = 2 * np.sum(Pvv0 * dPvv_an)
inv_fd = 2 * np.sum(Pvv0 * dPvv_fd)
print(
    "  ∂Tr[Pvv²]: an=%.6f fd=%.6f ratio=%.4f"
    % (inv_an, inv_fd, inv_an / inv_fd if abs(inv_fd) > 1e-12 else 0)
)
...
def analytic_alpha_explicit():
    ...
    Pvv0 = 2 * np.einsum("ijca,ijcb->ab", tij, tij) - np.einsum(
        "ijca,ijbc->ab", tij, tij
    )
    dmP0 = np.zeros((nmo, nmo))
    dmP0[O, O] = Poo0 + Poo0.T
    dmP0[Vv, Vv] = Pvv0 + Pvv0.T
    L0 = lagrangian_es(t)
    J = np.einsum("pqrs,rs->pq", Imo, dmP0)
    K = np.einsum("prqs,rs->pq", Imo, dmP0)
    Xvo0 = L0 + (2 * J - K)[Vv, O]
    ...
    for q in range(3):
        ...
        for a in range(nvir):
            for i in range(nocc):
                dDscf[nocc + a, i] += 2 * U[a, i]
                dDscf[i, nocc + a] += 2 * U[a, i]
        Gd = 2 * np.einsum("pqrs,rs->pq", Imo, dDscf) - np.einsum(
            "prqs,rs->pq", Imo, dDscf
        )
        ...
        def Xvo_at(s):
            ts = t2_amp(Imo + s * dImo, e + s * deps)
            tijs = ts.transpose(0, 2, 1, 3)
            Poo = -(
                2 * np.einsum("ikab,jkab->ij", tijs, tijs)
                - np.einsum("ikab,jkba->ij", tijs, tijs)
            )
            Pvv = 2 * np.einsum("ijca,ijcb->ab", tijs, tijs) - np.einsum(
                "ijca,ijbc->ab", tijs, tijs
            )
            dmP = np.zeros((nmo, nmo))
            dmP[O, O] = Poo + Poo.T
            dmP[Vv, Vv] = Pvv + Pvv.T
            Ls = lagrangian_es(ts)
            Is = Imo + s * dImo
            J = np.einsum("pqrs,rs->pq", Is, dmP)
            K = np.einsum("prqs,rs->pq", Is, dmP)
            ...
            for a in range(nvir):
                for i in range(nocc):
                    for b in range(nvir):
                        for j in range(nocc):
                            Mm[a, i, b, j] = (
                                4 * Is[nocc + a, i, nocc + b, j]
                                - Is[nocc + a, nocc + b, i, j]
                                - Is[nocc + a, j, nocc + b, i]
                            )
        ...
        dD = np.zeros((nmo, nmo))
        dD[O, O] = dPoo + dPoo.T
        dD[Vv, Vv] = dPvv + dPvv.T
        for a in range(nvir):
            for i in range(nocc):
                dD[nocc + a, i] += 2 * U[a, i] + dz[a, i]
                dD[i, nocc + a] += 2 * U[a, i] + dz[a, i]
        for p in range(3):
...
def D_and_dEdF(F, axis):
    ...
    D_ao = Cf @ D @ Cf.T
    mu = (
        -np.einsum("pq,pq->", dip_ao[axis], D_ao) + nucl[axis]
    )  # -Tr[D r]+nuc... but this IS the dipole
    return mu


# dE/dF numerically and -Tr[D_relax r] at same F:
F = 0.0
dEdF = -(etot_field_y(1e-4) - etot_field_y(-1e-4)) / (
    2e-4
)  # = -dE/dF... E(F)=E0-F<r> so dE/dF=-<r>=-mu?
...
def etot_z(F):
    m = scf.RHF(mol)
    m.get_hcore = lambda *a: hcore_ao - F * dip_ao[2]
    m.conv_tol = 1e-12
    m.kernel()
    return m.e_tot + mp.MP2(m).run().e_corr
...
for ax, nm in [(0, "x"), (1, "y"), (2, "z")]:
    ...
    def ez(F):
        m = scf.RHF(mol)
        m.get_hcore = lambda *a: hcore_ao - F * dip_ao[ax]
        m.conv_tol = 1e-12
        m.kernel()
        return m.e_tot + mp.MP2(m).run().e_corr

    mu_E = (
        (ez(1e-4) - ez(-1e-4)) / (2e-4) * (-1) * (-1)
    )  # μ=-dE/dF, dE/dF=(ez(h)-ez(-h))/2h
...
D_ps_relaxed = dm1_mo.copy()
D_ps_relaxed[Vv, O] += z_ps
D_ps_relaxed[O, Vv] += z_ps.T
D_me = static_relaxed_dm_validated(Imo, e)
print("  ‖D_me - D_ps_relaxed‖ =", np.max(np.abs(D_me - D_ps_relaxed)))
print(
    "  block diffs: oo=%.2e vv=%.2e ov=%.2e"
    % (
