...
def main():
    ...
    print(
        f"{'mol':6} | {'lit-exp':>7} {'fer-exp':>7} | {'CCSDT':>6} {'litGW@HF':>8} "
    ...
    for m, cas in CAS.items():
        ...
        print(
            f"{m:6} | {le if le else float('nan'):7.2f} {fe:7.2f} | "
    ...
    print("-" * 84)
    print(
        f"ferric evGW vs CCSD(T):  MAE={statistics.mean(abs(x) for x in dferr):.3f}  "
        f"ME={statistics.mean(dferr):+.3f}  N={len(dferr)}"
    )
    print(
        f"lit GW@HF  vs CCSD(T):  MAE={statistics.mean(abs(x) for x in dlerr):.3f}  "
        f"ME={statistics.mean(dlerr):+.3f}  N={len(dlerr)}"
    )
    print(
        f"ferric exp ip_ref vs GW100-db experimental: max|Δ|={max(expmis):.3f} eV "
    ...
    for m, cas in CAS.items():
        ...
        if lg:
            ...
            if m not in ("N2", "CO"):
                dvl_clean.append(d)
    print(
        f"ferric evGW vs lit GW@HF: MAE={statistics.mean(abs(x) for x in dvl):.3f}  "
