...
for line in (HERE / "kmols_def2tzvp_rows.txt").read_text().splitlines():
    m = re.match(
        r"^(BrK|HK|K2)\s+([-+0-9.]+)\s+" + r"\s+".join(r"([-+0-9.]+)" for _ in METHODS),
        line.strip(),
    )
    if m:
        g = m.groups()
...
for b in ("aug-cc-pvdz", "aug-cc-pvtz"):
    ...
    d["failure_reasons"] = {
        k: v
        for k, v in d.get("failure_reasons", {}).items()
        if k in set(d.get("failed", []))
    }
    tmp = p.with_suffix(".json.tmp")
    tmp.write_text(json.dumps(d, indent=2, sort_keys=True))
    tmp.replace(p)
    print(
        f"  {b}: injected {sorted(rows)} -> {len(d['molecules'])} conv, failed={sorted(d.get('failed', []))}"
    )
