...
occ_mo = list(range(nocc_s))
vir_mo = list(range(nocc_s, nao))
    ...
    for p in range(nP):
        for q in range(nQ):
            for r in range(nR):
                for s in range(nS):
                    v = 0.0
                    if spn(p) == spn(r) and spn(q) == spn(s):
                        v += eri[spP[p // 2], spR[r // 2], spQ[q // 2], spS[s // 2]]
                    if spn(p) == spn(s) and spn(q) == spn(r):
                        v -= eri[spP[p // 2], spS[s // 2], spQ[q // 2], spR[r // 2]]
                    out[p, q, r, s] = v
    return out
...
t1s = mycc.t1
t2s = mycc.t2  # t1s[i,a], t2s[i,j,a,b] spatial (alpha=beta)
t1 = np.zeros((no, nv))
for i in range(no):
    for a in range(nv):
        if spn(i) == spn(a):
    ...
    for j in range(no):
        for a in range(nv):
            for b in range(nv):
                ...
                val = 0.0
                if spn(i) == spn(a) and spn(j) == spn(b):
                    val += t2s[i // 2, j // 2, a // 2, b // 2]
                if spn(i) == spn(b) and spn(j) == spn(a):
                    val -= t2s[i // 2, j // 2, b // 2, a // 2]
                t2[i, j, a, b] = val
d3 = (
...
t3c = np.einsum("jkae,bcei->ijkabc", t2, bcei) - np.einsum(
    "imbc,majk->ijkabc", t2, majk
)
