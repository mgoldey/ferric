...
INPUTS = {
    "baseline_deficit_DZ": (
        219.0,
        "MEASURED-numerator / ASSUMED-denominator",
    ...
    "screen_gain": (
        1.74,
        "MEASURED (isolated) / MEASURED-to-compose (this work)",
...
def main():
    ...
    #      kernel as labelled.
    a_dz = show(
        "SCENARIO A -- HONEST (measured fit, criterion-satisfying grid)",
        [
            ("fit (measured on rxn energy)", 1.0, "MEASURED"),
            ("right-sized grid", 1.00, "MEASURED"),
            ("screening", 1.74, "MEASURED"),
            ("MD 3c1e kernel", 7.1, "ASSUMED"),
        ],
        dz,
    )
    a_tz = show(
        "SCENARIO A -- HONEST, cc-pVTZ",
        [
            ("fit (measured on rxn energy)", 1.0, "MEASURED"),
            ("right-sized grid", 1.00, "MEASURED"),
            ("screening", 1.85, "MEASURED"),
            ("MD 3c1e kernel", 3.8, "ASSUMED"),
        ],
        tz,
    )
    ...
    b_dz = show(
        "SCENARIO B -- UPPER BOUND ON BENEFIT (water-best fit, assumed to generalize)",
        [
