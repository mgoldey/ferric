#!/bin/bash
# T-formulation scan, r0 = 1.4-1.8 step 0.1 (EXTENSION), aQZ CP, 7-dimer subset.
...
run() {
  ...
  if [ -s "$out" ] && [ "$(grep -c 'Total energy' "$out")" -ge 5 ]; then
  ...
  if [ $rc -eq 0 ] && [ "$n" -ge 5 ]; then echo "[ok   ] ${el}s ${n}pts $key"; OK=$((OK+1))
...
for s in 20 21 12 24 15 22 23; do
  for f in dimer mA_cp mB_cp; do run "a24-${s}_${f}_aqz_r0text_T"; done
done
