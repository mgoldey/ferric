...
def trans(eri, mos):
    ...
    e = numpy.dot(mos[0].T, eri.reshape(s[0][0], -1)).reshape(
        s[0][1], s[1][0], s[2][0], s[3][0]
    )
    e = (
        numpy.dot(mos[1].T, e.transpose(1, 0, 2, 3).reshape(s[1][0], -1))
        .reshape(s[1][1], s[0][1], s[2][0], s[3][0])
        .transpose(1, 0, 2, 3)
    )
    e = (
        numpy.dot(e.transpose(0, 1, 3, 2).reshape(-1, s[2][0]), mos[2])
        .reshape(s[0][1], s[1][1], s[3][0], s[2][1])
        .transpose(0, 1, 3, 2)
    )
    e = numpy.dot(e.reshape(-1, s[3][0]), mos[3]).reshape(
...
try:
    ...
    Y = trans(mol.intor("int2e_yp"), [mo_o] * 4)
    ...
    Rbar = cabs_mol.intor("int2e_stg", shls_slice=cbs + obs + obs + obs).reshape(
        nca, nao, nao, nao
    )
    Rbar_minj = trans(Rbar[:nao], [mo_o] * 4)
    v = cabs_mol.intor("int2e", shls_slice=cbs + obs + obs + obs).reshape(
        nca, nao, nao, nao
    )
    ...
    eV = (
        numpy.einsum("minj,minj", V, tminj) * 4
        - numpy.einsum("minj,nimj", V, tminj) * 2
    )
    X = Rbar_minj.copy()
    ...
    eX = -numpy.einsum("mn,minj,minj", e_mn, tmp, tminj) * 2 + numpy.einsum(
        "mn,minj,nimj", e_mn, tmp, tminj
    )
    print("NE_RHF", mf.e_tot)
    print("NE_dims nocc", nocc, "nmo", nmo, "nca", nca)
    print("NE_F12_V", eV)
    print("NE_F12_X", eX)
    print("NE_F12_VX", eV + eX)
except Exception:
    traceback.print_exc()
