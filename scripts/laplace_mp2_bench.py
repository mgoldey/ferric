...
def run_bench(n_carbon):
    ...
    t0 = time.time()
    res = ferric.run_laplace_mp2(
        mol, bs, aux, n_quad=7, frozen_core=frozen_core, k_builder="link"
    )
...
if __name__ == "__main__":
    print(
        "AO Laplace RI-MP2 Scaling Benchmark: Linear Alkanes, cc-pVDZ/cc-pVDZ-RI, 7-point quadrature"
    )
    print("-" * 80)
    ...
    for n in range(1, 8):
        try:
            ...
            print(
                f"{n:<6} {elapsed:>10.2f} {res.total_energy - res.mp2_corr:>18.8f} {res.mp2_corr:>16.8f}"
            )
