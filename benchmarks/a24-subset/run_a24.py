...
def frags(idx):
    ...
    for p in parts:
        lines = [
            ...
            if l.strip() and not l.startswith(("0 1", "units"))
        ]
        atoms = [
            l.split() for l in lines if re.match(r"\s*[A-Za-z]", l) and "units" not in l
        ]
        atoms = [
            (a[0], float(a[1]), float(a[2]), float(a[3])) for a in atoms if len(a) == 4
        ]
        out.append(atoms)
...
def run(tomlpath):
    ...
    r = subprocess.run(
        [BIN, tomlpath], capture_output=True, text=True, env=env, timeout=1800
    )
...
for idx in REFS:
    ...
    for tag, atoms in geos.items():
        wxyz(f"{sysd}/{tag}.xyz", atoms, f"A24-{idx} {NAMES[idx]} {tag}")
    for w in OMEGAS:
        for tag in geos:
            tp = f"{sysd}/{tag}_w{w}.toml"
            toml(tp, f"{sysd}/{tag}.xyz", w)
            e = run(tp)
            results[(idx, w, tag)] = e
...
print(
    f"{'system':12s} {'omega':>5s} {'RHF':>8s} {'MP2':>8s} {'naiveA':>8s} {'DeltaB':>8s} {'ref':>7s}"
)
...
for idx in REFS:
    for w in OMEGAS:
        ...
        for m in ("rhf", "mp2", "A", "B"):
            d, a, b = (
                results[(idx, w, "dimer")][m],
                results[(idx, w, "mA_cp")][m],
                results[(idx, w, "mB_cp")][m],
            )
            cp = (d - a - b) * K
            ...
            row.append((cp, ncp))
            if m != "rhf":
                err[m][w].append(cp - REFS[idx])
        print(
            f"{NAMES[idx]:12s} {w:>5s} "
            + " ".join(f"{c:8.3f}" for c, _ in row)
            + f" {REFS[idx]:7.3f}   (non-CP: "
            + " ".join(f"{n:7.3f}" for _, n in row)
            + ")"
        )
