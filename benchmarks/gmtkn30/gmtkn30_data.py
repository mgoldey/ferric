...
def read_charge(filename, dir="."):
    ...
    for line in lines:
        sline = line.split()
        charge.append((sline[0], float(re.search("\(([-+]\d+)\)", sline[1]).group(1))))
    fh.close()
    return charge
        ...
        if item.find("TTDD") != -1:
            ...
            for f in item.split(separator):
                ...
                try:
                    ...
                    if (
                        fs.isdigit() and str(v) != fs
                    ):  # e.g. undesirable eval('001') = 1
                        v = fs
            ...
            if ncompounds:
                assert ncompounds == (len(litem) - 2) / 2, (
                    "Error: number of compounds incorrect for reaction: "
                    + str(litem[0])
                    + " in file: "
                    + filename
                )
