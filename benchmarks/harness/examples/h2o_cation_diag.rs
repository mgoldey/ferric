...
fn s_squared(
    c_a: &ndarray::Array2<f64>,
    c_b: &ndarray::Array2<f64>,
    s: &ndarray::Array2<f64>,
    na: usize,
    nb: usize,
) -> f64 {
...
fn main() {
    ...
    // Neutral RHF first to get directed guess.
    {
        ...
        let res = solve_uhf_with_guess(&ctx, &cation, &prep, &bounds, &cfg, Some((&c_rhf, &c_rhf)))
            .unwrap();
        println!(
            "[neutral-RHF-guess UHF] E={:.8}, conv={}, iters={}",
    ...
    for max_iter in [1usize, 2, 3, 5, 10, 20, 50] {
        ...
        match solve_uhf(&ctx, &cation, &prep, &bounds, &cfg) {
            Ok(r) => println!(
                "[max_iter={:>3}] E={:.6}, conv={}, iters={}",
                max_iter, r.energy, r.converged, r.iterations
            ),
            Err(e) => match e {
                ferric_core::FerricError::ScfConvergence {
                    iterations,
                    last_energy,
                } => {
                    println!(
                        "[max_iter={:>3}] (DNF) last_E={:.6}, iters={}",
                        max_iter, last_energy, iterations
                    );
                }
    ...
    println!(
        "  α eps[0..6] = {:?}",
        &res.eps_alpha[..6.min(res.eps_alpha.len())]
    );
    println!(
        "  β eps[0..6] = {:?}",
        &res.eps_beta.as_ref().unwrap()[..6.min(res.eps_beta.as_ref().unwrap().len())]
    );
