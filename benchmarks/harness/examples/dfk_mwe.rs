...
fn main() {
    let which = std::env::args()
        .nth(1)
        .unwrap_or_else(|| "benzene-atz".into());
    ...
    let n = obs.nbasis();
    let naux = auxp.nbasis();
    let nocc = (mol.nelec() / 2) as usize;
    println!("{which}: n={n} naux={naux} nocc={nocc}");

    let t = Instant::now();
    let mut dfk = ferric_scf::df_k::DfK::new(
        op,
        &obs,
        &auxp,
        ferric_scf::rhf::resolve_three_index_budget(0),
    )
    .unwrap();
    println!(
        "  DfK::new (fit+dress, one-time): {:.0} ms",
    ...
    println!(
        "  build_from_occ  {:8.1} ms   ~{:6.1} GFLOP/s   speedup {:.2}x",
        t_occ * 1e3,
        fl_occ / t_occ / 1e9,
        t_den / t_occ
    );
