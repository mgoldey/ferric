...
fn main() {
    ...
    println!(
        "SchwarzBounds        : {:8.0} ms",
        t.elapsed().as_secs_f64() * 1e3
    );

    // The DF-JK 3-index tensor over the JK aux basis: built once, reused each iter.
    let t = Instant::now();
    let n = threeindex::eri3_tensor(op, &obs, &jk)
        .map(|a| a.len())
        .unwrap_or(0);
    println!(
        "eri3(JK aux) once    : {:8.0} ms  ({} elems)",
        t.elapsed().as_secs_f64() * 1e3,
        n
    );
    ...
    println!(
        "solve_rhf total      : {:8.0} ms  ({} iters, {:.1} ms/iter)",
        tot * 1e3,
        r.iterations,
        tot * 1e3 / r.iterations as f64
    );
