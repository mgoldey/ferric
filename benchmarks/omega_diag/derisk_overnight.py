...
while True:
    ...
    waited += 30
    if waited > 3600:
        log("non-CP sweep >1h, proceeding anyway")
        break
...
for basis, aux, btag in ADZ + ATZ:
    for sid, label in ANCHORS:
        ...
        rhf_ca = rhf_energy(
            f"{GEO}/s22-{sid}_mA_cp.xyz",
            basis,
            aux,
            fc_count(f"{GEO}/s22-{sid}_mA_cp.xyz"),
            f"{label}_{sid}_{btag}_RHF_cpA",
        )
        rhf_cb = rhf_energy(
            f"{GEO}/s22-{sid}_mB_cp.xyz",
            basis,
            aux,
            fc_count(f"{GEO}/s22-{sid}_mB_cp.xyz"),
            f"{label}_{sid}_{btag}_RHF_cpB",
        )
        for omega in OMEGAS:
            kB = f"{label}_{sid}_{btag}_w{omega}_B"
            kT = f"{label}_{sid}_{btag}_w{omega}_T"

            def corr(frag_key, pat):
                return grab(frag_key, pat)
            ...
            def bind(method, arm):
                if arm == "noncp":
                    rd, ra, rb = rhf_d, rhf_pa, rhf_pb
                    fa, fb = "A", "B"
                else:
                    rd, ra, rb = rhf_d, rhf_ca, rhf_cb
                    fa, fb = "cpA", "cpB"
                if None in (rd, ra, rb):
                    return None
                if method in pats:
...
json.dump(
    ...
    open(f"{OUT}/derisk_results.json", "w"),
    indent=1,
)
log(
    f"wrote {OUT}/derisk_results.json with {sum(1 for v in results.values() if v is not None)} values"
)

# Markdown report
lines = [
    "# De-risk: CP vs non-CP × method × interaction-type (aDZ, aTZ)\n",
