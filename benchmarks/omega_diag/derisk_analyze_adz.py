...
def split_monomers(sid):
    ...
    pa = f"{OUT}/s22-{sid}_A.xyz"
    pb = f"{OUT}/s22-{sid}_B.xyz"
...
"""
    ...
    with open(op, "w") as f, open(op + ".err", "w") as e:
        subprocess.run(
            [BIN, f"{OUT}/toml/{key}.toml"], stdout=f, stderr=e, env=env, timeout=10800
        )
    m = re.search(r"RHF energy\s*=\s*(-?[0-9.]+)", open(op).read())
    return float(m.group(1)) if m else None
...
for basis, aux, btag in ADZ:
    for sid, label in ANCHORS:
        ...
        rhf_d = rhf_energy(
            dim, basis, aux, fc_count(dim), f"{label}_{sid}_{btag}_RHF_dimer"
        )
        ...
        rhf_ca = rhf_energy(
            cpA, basis, aux, fc_count(cpA), f"{label}_{sid}_{btag}_RHF_cpA"
        )
        rhf_cb = rhf_energy(
            cpB, basis, aux, fc_count(cpB), f"{label}_{sid}_{btag}_RHF_cpB"
        )
        for omega in OMEGAS:
            ...
            def piece(fr, pat):
                return grab(f"{kB}_{fr}", pat)

            def bind(method, arm):
                if arm == "noncp":
                    rd, ra, rb, fa, fb = rhf_d, rhf_pa, rhf_pb, "A", "B"
                else:
                    rd, ra, rb, fa, fb = rhf_d, rhf_ca, rhf_cb, "cpA", "cpB"
                if None in (rd, ra, rb):
                    return None
                if method in pats:
                    cd = piece("dimer", pats[method])
                    ca = piece(fa, pats[method])
                    cb = piece(fb, pats[method])
                    if None in (cd, ca, cb):
                        return None
            ...
            for method in ["MP2", "SRMP2", "naiveA", "B", "T"]:
                for arm in ["noncp", "cp"]:
                    results[f"{label}|{sid}|{btag}|{omega}|{method}|{arm}"] = bind(
                        method, arm
                    )

json.dump(
    {"binds": results, "ccsdt": CCSDT},
    open(f"{OUT}/derisk_results_adz.json", "w"),
    indent=1,
)
...
for sid, label in ANCHORS:
    ...
    L.append("| ω | method | non-CP | err | CP | err |")
    L.append("|---|---|---|---|---|---|")
    for omega in OMEGAS:
        for method in ["MP2", "SRMP2", "naiveA", "B", "T"]:
...
for omega in OMEGAS:
    for method in ["MP2", "SRMP2", "naiveA", "B", "T"]:
        sn = stats(method, "noncp", omega)
        sc = stats(method, "cp", omega)

        def f(s):
            return (f"{s[0]:.3f}", f"{s[1]:+.3f}") if s else ("—", "—")
...
print(
    f"wrote DERISK_REPORT_adz.md + derisk_results_adz.json ({nvals}/{len(results)} binds)"
)
