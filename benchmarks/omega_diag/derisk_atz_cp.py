...
for sid, label in [a for a in ANCHORS if a[0] != "11"] + [
    a for a in ANCHORS if a[0] == "11"
]:
    frags = {
        ...
        for omega in OMEGAS:
            for form, ftag in FORMS:
                write_run(
                    ...
                    f"{label}_{sid}_{BT}_w{omega}_{ftag}_{fr}",
                    "Total energy",
                )
...
for sid, label in ANCHORS:
    ...
    cpAx = f"{GEO}/s22-{sid}_mA_cp.xyz"
    cpBx = f"{GEO}/s22-{sid}_mB_cp.xyz"
    ...
    for omega in OMEGAS:
        ...
        for method in ["MP2", "SRMP2", "naiveA", "B", "T"]:
            ...
            if method in PIECE:
                ...
                ca = grab(out(f"{kB}_cpA"), PIECE[method])
                cb = grab(out(f"{kB}_cpB"), PIECE[method])
                if None not in (rd, rA, rB, cd, ca, cb):
                    v = ((rd + cd) - (rA + ca) - (rB + cb)) * KCAL
            else:
                k = kB if method == "B" else kT
                td = grab(out(f"{k}_dimer"), TOT)
                ta = grab(out(f"{k}_cpA"), TOT)
                tb = grab(out(f"{k}_cpB"), TOT)
                if None not in (td, ta, tb):
                    v = (td - ta - tb) * KCAL
            results[f"{label}|{sid}|{BT}|{omega}|{method}|cp"] = v

json.dump(
    {"binds": results, "ccsdt": CCSDT}, open(f"{OUT}/derisk_atz_cp.json", "w"), indent=1
)
...
def curve(method):
    ...
    for omega in OMEGAS:
        ...
        if errs:
            ...
            L.append(
                f"| {omega} | {len(errs)} | {mae:.3f} | {mse:+.3f} | {bz:+.3f if bz is not None else 0} |".replace(
                    "+0.000 if bz is not None else 0", "—"
                )
            )
            # simpler bz formatting:
            L[-1] = (
                f"| {omega} | {len(errs)} | {mae:.3f} | {mse:+.3f} | {(f'{bz:+.3f}' if bz is not None else '—')} |"
            )
        else:
            L.append(f"| {omega} | 0 | — | — | — |")
    return L
...
R = [
    "# aTZ CP ω-selection for SR-MP2+LR-RPA (B,T)\n",
