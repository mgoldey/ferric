...
for r0 in sorted(data):
    ...
    for s, fr in data[r0].items():
        if len(fr) < 3 or s not in bind:
            continue
        b = (fr["dimer"][0] - fr["mA_cp"][0] - fr["mB_cp"][0]) * K
        mp = (fr["dimer"][1] - fr["mA_cp"][1] - fr["mB_cp"][1]) * K
        eb.append(abs(b - bind[s]))
        em.append(abs(mp - bind[s]))
    if len(eb) < 24:
        continue
    mb, mm = sum(eb) / len(eb), sum(em) / len(em)
    print(
        f"{r0:6.2f} {len(eb):3} {mb:9.4f} {mm:9.4f} {mm - mb:+9.4f} {100 * (mm - mb) / mm:6.1f}%"
    )
