...
# ---------------------------------------------------------------- databases
PSI4_RAW = (
    "https://raw.githubusercontent.com/psi4/psi4/master/psi4/share/psi4/databases"
)
    ...
    for z, ed in d["elements"].items():
        ...
        for sh in ed["electron_shells"]:
            for i, l in enumerate(sh["angular_momentum"]):
                ncon = (
                    len(sh["coefficients"]) if len(sh["angular_momentum"]) == 1 else 1
                )
                n += (2 * l + 1) * ncon
        out[int(z)] = n
    return out
...
def estimate(atoms, basis):
    ...
    est = (
        BASE
        + AO_MULT * min(naux * nbf * nbf * 8, BUDGET)
        + 3 * naux * nia * 8
        + nvir * nia * 8
    )
    if basis == "aqz":
...
def toml_text(xyz, basis, method, trunc, scf_extra=""):
    ...
    if method == "scs":
        ...
        form, omega = (
            ("delta-lr", "0.42") if method == "dlr042" else ("coupled-rings", "0.2")
        )
        t += f'[method]\nkind = "rs-mp2-rpa"\n\n'
        t += f'[mp2]\nauxbasis = "{aux}"\nomega = {omega}\nformulation = "{form}"\n'
...
def build_jobs():
    ...
    json.dump(
        {"a24": a24_refs, "s22": s22_refs, "s66": s66_refs},
        open(ROOT / "refs.json", "w"),
        indent=1,
    )
    ...
    for dbse, geos in (("a24", a24_geos), ("s22", s22_geos), ("s66", s66_geos)):
        for idx, (fA, fB) in sorted(geos.items()):
            ...
            for tag, atoms in frags.items():
                ...
                geom_drift = write_if_changed(
                    xyz_path, xyz_text(atoms, f"{sysname} {tag}")
                )
                if geom_drift:
                ...
                for basis in BASES:
                    ...
                    for method in METHODS:
                        ...
                        if est > JOB_CAP:
                            ...
                            continue
                        tt = toml_text(
                            xyz_path, basis, method, trunc, SCF_EXTRA.get(sysname, "")
                        )
...
def launch(job, blas_threads=1, rayon_threads=1):
    ...
    proc = subprocess.Popen(
        [BIN, str(ROOT / "toml" / f"{key}.toml")],
        stdout=part,
        stderr=err,
        env=env,
        preexec_fn=make_preexec(limit),
        cwd=ROOT,
    )
    log(
        f"start {key} est={job['est'] / GB:.1f}G nbf={job['nbf']} "
...
def finish(rec):
    ...
    if job["attempts"] <= 2:
        ...
        log(
            f"requeue {key} rc={rc} attempt={job['attempts']} est->{job['est'] / GB:.1f}G"
        )
        return "requeue"
...
def validate_trunc():
    ...
    est, _, _ = estimate(
        [
            (t.split()[0], 0, 0, 0)
            for t in xyz.read_text().splitlines()[2:]
            if t.split()
        ],
        "adz",
    )
    ...
    while (
        subprocess.run(
            ["pgrep", "-f", "val_benzene_fullrank.toml"], capture_output=True
        ).returncode
        == 0
    ):
    ...
    if not (tr_out.exists() and "Total energy" in tr_out.read_text()):
        ...
        run_sync(
            ROOT / "toml" / f"{tr_key}.toml", ROOT / "out" / f"{tr_key}.out.part", est
        )
...
def main():
    ...
    prefixes = [
        p for p in os.environ.get("GRID_KEY_PREFIXES", "").split(",") if p.strip()
    ]
    ...
    todo = [
        j
        for j in jobs
        if not is_done(j) and not (ROOT / "out" / f"{j['key']}.failed").exists()
    ]
    log(
        f"jobs: {len(jobs)} total, {len(jobs) - len(todo)} done, "
    ...
    if not validated:
        trunc_gated = lambda j: (
            j["key"].startswith(("s22", "s66")) and j["method"] != "scs"
        )
        n_defer = sum(1 for j in todo if trunc_gated(j))
        log(
            f"trunc not yet validated: deferring {n_defer} S22/S66 rs jobs "
    ...
    while pending or running:
        ...
        if running and mem_available() < FLOOR:
            ...
            log(
                f"WATCHDOG: MemAvailable<{FLOOR / GB:.1f}G — killing {victim['job']['key']}"
            )
            kill_group(victim)
        ...
        json.dump(
            dict(
                pending=len(pending),
                running=[r["job"]["key"] for r in running],
                done=n_done,
                failed=n_fail,
                t=time.time(),
            ),
            open(ROOT / "status.json", "w"),
            indent=1,
        )
        time.sleep(POLL)
