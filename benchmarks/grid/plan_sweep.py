            ...
            rows.append(
                {
                    ...
                    "status": "complete"
                    if not need
                    else ("partial" if have else "missing"),
                }
            )
    return rows
...
def main():
    ...
    ap.add_argument(
        "--template", default=None, help="TOML to clone (default: an existing sweep)"
    )
    ap.add_argument("--out", default=None, help="plan JSON path")
    ap.add_argument(
        "--write", action="store_true", help="also create/trim/remove TOMLs"
    )
    a = ap.parse_args()

    r0_want = [round(float(x), 4) for x in a.r0.split(",")]
    systems = (
        [int(x) for x in a.systems.split(",") if x.strip()]
        if a.systems
    ...
    doc = {
        ...
        "note": (
            "Re-run this to replan; it recomputes from outputs on disk, so "
    ...
    out = (
        Path(a.out)
        if a.out
        else ROOT / "scans" / f"plan_{a.basis}_{a.form}_{a.tag}.json"
    )
    ...
    print(f"wrote {out}")
    print(
        f"  jobs: {s['jobs_total']} total / {s['jobs_complete']} complete / "
        f"{s['jobs_partial']} partial / {s['jobs_missing']} missing"
    )
    print(
        f"  r0 points: {s['r0_points_already_on_disk']} on disk, "
    ...
    tpl_path = (
        Path(a.template)
        if a.template
        else ROOT / "toml" / "a24-12_dimer_aqz_r0fine_B.toml"
    )
    ...
    for r in rows:
        ...
        t = re.sub(
            r'formulation = "[^"]*"', f'formulation = "{FORMULATION[a.form]}"', t
        )
    ...
    print(
        f"  TOMLs: {created} created, {trimmed} updated "
