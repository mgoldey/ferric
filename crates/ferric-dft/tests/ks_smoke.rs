...
fn ks_xc_b3lyp_kmix_is_plain_hybrid() {
    ...
    assert!(
        mix.sr > 0.0 && mix.sr < 1.0,
        "B3LYP has partial exact exchange: sr = {}",
        mix.sr
    );
    assert!(
        (mix.lr - mix.sr).abs() < 1e-12,
        "B3LYP: sr == lr when omega=0"
    );
    // B3LYP exact-exchange fraction is ~0.20; loosen to 0.05..0.30 for libxc version tolerance
    assert!(
        mix.sr > 0.05 && mix.sr < 0.30,
        "B3LYP exact-exchange mix ≈ 0.20, got {}",
        mix.sr
    );
}
...
fn ks_xc_add_xc_produces_negative_energy_h2o_lda() {
    ...
    assert!(
        e_xc < -3.0 && e_xc > -15.0,
        "E_xc out of expected range: {e_xc}"
    );
