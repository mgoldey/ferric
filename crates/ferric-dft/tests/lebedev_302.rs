...
fn lebedev_302_has_302_points_and_unit_weight_sum() {
    ...
    assert_eq!(
        pts.len(),
        302,
        "Lebedev-302 should produce exactly 302 points, got {}",
        pts.len()
    );
...
fn lebedev_302_points_are_unit_norm() {
    ...
    assert!(
        max_err < 1e-13,
        "Lebedev-302 points not unit-norm: max err {max_err:.2e}"
    );
}
...
fn lebedev_302_integrates_polynomials_exactly() {
    ...
    let int_x2: f64 = pts
        ...
        .sum();
    assert!(
        (int_x2 - 1.0 / 3.0).abs() < 1e-12,
        "∫x²/4π = {int_x2}, expect 1/3"
    );

    // ∫_sphere x²y²z² dΩ / 4π = 1/105 (standard result for L=6)
    let int_x2y2z2: f64 = pts
        .iter()
        .zip(wts.iter())
        .map(|(p, w)| w * (p[0] * p[1] * p[2]).powi(2))
        .sum();
    assert!(
        (int_x2y2z2 - 1.0 / 105.0).abs() < 1e-13,
    ...
    let int_x6: f64 = pts
        .iter()
        .zip(wts.iter())
        .map(|(p, w)| w * p[0].powi(6))
        .sum();
    assert!(
        (int_x6 - 1.0 / 7.0).abs() < 1e-13,
        "∫x⁶/4π = {int_x6}, expect 1/7"
    );
