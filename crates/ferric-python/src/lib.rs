...
/// include every polarizable-embedding force term as of Lane B5**: a
/// polarizable site's own `dE_pol/dR_site` (`ferric_scf::qmmm::
/// polarizable_site_gradient`) AND the reaction force every embedding
/// charge feels from the OTHER sites' induced dipoles
/// (`ferric_scf::qmmm::polarizable_charge_gradient_rows`, wired via
/// `ferric_scf::polarizable::charge_gradient_contribution` — see that
/// function's doc for why this is a distinct term from the site's own row)
/// are both added into `full_gradient()`'s output here, for every method
/// (this part of the pipeline is method-agnostic — only the QM-region
/// Fock-term gradient above is `rhf`-only today). Verified against central
/// finite differences of the fully-reconverged total energy on a colocated
/// charge+alpha MM atom (the realistic force-field case) and a two-site
/// mutual-induction case — see `crates/ferric-scf/tests/
/// qmmm_polarizable_forces.rs`. Boundary charges (RC/RCD) combined with a
/// polarizable site are NOT covered by an FD test (documented gap in that
/// same test file, not a silent one) — the arithmetic (half the reaction
/// force onto each host) is implemented and follows the same convention
/// `full_gradient` already uses for `mm_forces`, but is unverified in
/// combination.
#[pyfunction]
