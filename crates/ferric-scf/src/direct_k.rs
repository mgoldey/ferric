...
impl<'a> KBuilder for DirectK<'a> {
    fn build(&mut self, d: &Array2<f64>, k: &mut Array2<f64>) -> Result<usize, FerricError> {
        ...
        let total_k = pairs_for_this_rank.into_par_iter().fold(
            || (
                Engine::new_2e(self.bounds.op, self.prep, 1e-14).unwrap(),
                Array2::zeros(k.raw_dim()),
                0usize
            ),
            |(mut engine, mut local_k, mut local_count), (s1, s2)| {
                ...
                for s3 in 0..=s1 {
                    ...
                    for s4 in 0..=s4max {
                        ...
                        if let Some(q) = engine.compute_quartet(self.prep, s1, s2, s3, s4) {
                            local_count += 1;
