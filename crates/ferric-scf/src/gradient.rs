...
pub fn rhf_gradient(
    ...
    // Energy-weighted density: W = D F D
    let w = d.dot(f).dot(d);
...
fn accum_2e_grad(
    ...
    atoms: &[usize; 4], s1: usize, s2: usize, s3: usize, s4: usize,
) {
    // dq has 12 blocks: 3 coords × 4 centers
    // For each (μν|λσ), the J contribution is D[λσ] and K contribution is D[νσ], D[μσ], etc.
    // We accumulate dJ/dR and -0.5 dK/dR per atom center.
    for a in 0..n1 {
        for b in 0..n2 {
            for c in 0..n3 {
                for dd in 0..n4 {
                    ...
                    // J prefactor: D[μν] D[λσ]
                    let pj = d[(mu, nu)] * d[(la, sg)];
                    // K prefactor: -0.5 * (D[μλ] D[νσ] + D[μσ] D[νλ])
                    // From exchange: K[μλ] += D[νσ] (μν|λσ), etc.
                    let pk = -0.5 * (d[(mu, la)] * d[(nu, sg)] + d[(mu, sg)] * d[(nu, la)]);

                    let pref = pj + pk;

                    // Symmetry factor for canonical loop
                    let sym = sym_factor(s1, s2, s3, s4);

                    // Accumulate into gradient for each center
                    for center in 0..4 {
                        for coord in 0..3 {
                            let dv = dq[(center * 3 + coord) * block_sz + idx];
                            grad[(atoms[center], coord)] += sym * pref * dv;
                        }
                    }
                }
            }
...
fn sym_factor(s1: usize, s2: usize, s3: usize, s4: usize) -> f64 {
    let mut f = 1.0;
    if s1 != s2 { f *= 2.0; }
    if s3 != s4 { f *= 2.0; }
    if (s1, s2) != (s3, s4) { f *= 2.0; }
    f
}
