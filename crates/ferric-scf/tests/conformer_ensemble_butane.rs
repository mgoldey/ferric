...
fn butane_ensemble_end_to_end_with_real_scf_energies() {
    ...
    eprintln!(
        "weights: anti = {:.10}, gauche = {:.10}",
        w.weights[0], w.weights[1]
    );
    ...
    assert!(
        stats.std_dev > 0.0,
        "a two-conformer spread must be non-zero"
    );
...
fn butane_single_conformer_ensemble_equals_the_single_point() {
    ...
    assert_eq!(
        w.weights,
        vec![1.0],
        "one conformer must have weight exactly 1.0"
    );
    ...
    assert_eq!(
        stats.std_dev, 0.0,
        "single-conformer spread must be EXACTLY zero"
    );
...
fn butane_duplicate_conformer_is_exactly_degenerate() {
    ...
    assert_eq!(
        w.weights[0], 0.5,
        "identical energies must give exactly 0.5"
    );
...
fn butane_conformer_ten_kt_up_is_negligible() {
    ...
    assert!(
        w.weights[1] < 5e-5,
        "10 kT above the minimum must be negligible"
    );
    ...
    assert!(
        (ten_kt_kcal - 5.925).abs() < 0.01,
        "10 kT should be ~5.925 kcal/mol"
    );
