...
use ferric_scf::cosx_k::{
    CosxConfig, CosxHalfTransform, CosxK, CosxTimings, COSX_DEFAULT_EPS_AO, COSX_DEFAULT_EPS_D,
};
...
fn setup_butane() -> Setup {
    let mol =
        Molecule::load_xyz(&testdata("testdata/molecules/alkane_4.xyz")).expect("alkane_4.xyz");
...
fn prod_screen() -> Option<f64> {
    ...
    assert!(
        matches!(t, Some(v) if v > 0.0),
        "the density-driven screen is expected ON by default"
    );
    t
}

const DENSE: CosxHalfTransform = CosxHalfTransform::Dense;
const SPARSE_ZERO: CosxHalfTransform = CosxHalfTransform::Sparse {
    eps_ao: 0.0,
    eps_d: 0.0,
};
const SPARSE_PROD: CosxHalfTransform = CosxHalfTransform::Sparse {
...
fn assert_all_active(t: &CosxTimings, label: &str) {
    ...
    assert_eq!(
        ...
        "{label}: eps = 0 dropped AOs from A (a floor on eps_ao)"
    );
    assert_eq!(
        t.lambda_frac, 1.0,
        "{label}: eps = 0 dropped rows from Λ (a floor on eps_d)"
    );
}
...
fn trivial_limit_on(s: &Setup, label: &str) {
    for fit in [true, false] {
        for (scale, tag) in [(1.0, "full"), (1e-8, "1e-8 x D")] {
            ...
            assert!(
                scale_k > 0.05 * scale,
                "{label} {tag}: K ~ 0 ({scale_k:.3e}); vacuous"
            );
            ...
            assert!(
                k_dense == k_zero,
                "{label} fit={fit} {tag}: eps = 0 is not bitwise the dense K (max dev {dev:.3e})"
            );
...
fn sparse_zero_eps_matches_dense_bitwise_butane() {
    ...
    for screen in [None, prod_screen()] {
        ...
        assert!(
            k_dense == k_zero,
            "butane, screen {screen:?}: eps = 0 not bitwise dense"
        );
        assert_all_active(&t, &format!("butane screen {screen:?}"));
        assert_eq!(
            (t.pairs_kept, t.pairs_total),
            (td.pairs_kept, td.pairs_total),
            "eps = 0 changed the pair screen's decisions"
        );
    }
}
...
fn production_eps_on(s: &Setup, label: &str) -> CosxTimings {
    ...
    assert!(
        dev < 1e-6,
        "{label}: sparse K differs from dense by {dev:.3e} >= 1e-6"
    );
    assert!(
        t.active_ao_frac < 1.0,
        "{label}: production eps dropped nothing — the accuracy claim is vacuous"
    );
...
fn sparse_cosx_rhf_water_energy_matches_dense() {
    ...
    assert!(
        de.abs() < 1e-7,
        "sparse COSX SCF energy differs from dense by {de:e} Ha"
    );
    assert_eq!(
        CosxConfig::default().half_transform,
        SPARSE_PROD,
        "sparse at the pre-registered eps must be the default"
    );
}
...
fn sparse_build_from_occ_is_the_density_path() {
    ...
    assert!(
        k_c == k_d,
        "sparse build_from_occ(C) is not bitwise build(C C^T): {:.3e}",
        max_abs_diff(&k_c, &k_d)
    );
    let mut kb_dense =
        CosxK::new(&ctx, &s.mol, &s.prep, cfg(DENSE, true, None), usize::MAX).expect("dense");
    let mut k_dense = Array2::zeros((n, n));
    kb_dense
        .build_from_occ(&s.c_occ, &mut k_dense)
        .expect("dense occ");
    let dev = max_abs_diff(&k_c, &k_dense);
    println!("build_from_occ sparse vs dense: max|dK| = {dev:.3e}");
    assert!(
        dev < 1e-6,
        "sparse build_from_occ vs dense differ by {dev:.3e}"
    );
}
...
fn sparse_reachability_counts_alkane_8() {
    let mol =
        Molecule::load_xyz(&testdata("testdata/molecules/alkane_8.xyz")).expect("alkane_8.xyz");
    ...
    assert!(
        dev < 1e-6,
        "alkane_8: sparse K differs from dense by {dev:.3e}"
    );
    assert!(
        t.active_ao_frac < 0.95,
        "alkane_8: mean |A|/nbf = {:.4} — the AO mask is not biting at all",
        t.active_ao_frac
    );
    assert!(
...
fn sparse_reachability_water_dimer_far() {
    ...
    assert!(
        dev < 1e-6,
        "dimer: sparse K differs from dense by {dev:.3e}"
    );
    assert!(
        t.active_ao_frac < 0.75,
        "dimer: mean |A|/nbf = {:.4} >= 0.75",
        t.active_ao_frac
    );
    assert!(
        t.lambda_frac < 0.75,
        "dimer: mean |Λ|/nbf = {:.4} >= 0.75 — the D-sparsity restriction is not biting",
        t.lambda_frac
    );
    ...
    assert!(
        t.out_ao_frac < 0.75,
        "dimer: mean |B|/nbf = {:.4} >= 0.75 — the touched-shell set is not biting",
        t.out_ao_frac
    );
    assert!(
        t.active_ao_frac_min <= 0.5 + 1e-12,
        "dimer: no block is confined to one monomer (min |A|/nbf {:.4})",
        t.active_ao_frac_min
    );
}
...
fn dense_counters_and_strict_parser() {
    ...
    assert_eq!(
        (t.active_ao_frac, t.lambda_frac, t.out_ao_frac),
        (1.0, 1.0, 1.0)
    );
    assert!(t.n_blocks > 0);
    ...
    assert!(
        CosxHalfTransform::parse_config_str("Dense").is_err(),
