//! FD validation of the range-separated hybrid (RSH) KS gradient path.
//!
//! Uses bare wB97X (libxc HYB_GGA_XC_WB97X) — the non-V variant, identical
//! to wB97X-V in the RSH/exchange structure but **without VV10**. This lets
//! us validate the c_SR · K_SR(ω) + c_LR · K_LR(ω) gradient piece in isolation.
//! For wB97X: ω=0.3, α=1.0, β=-0.842 → c_SR = α+β ≈ 0.158, c_LR = α = 1.0.
...
#[test]
fn wb97x_gradient_h2_sto3g_vs_fd() {
