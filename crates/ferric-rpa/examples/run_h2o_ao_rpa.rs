...
fn main() {
    ...
    let file_content = std::fs::read_to_string(grid_path)
        ...
        .unwrap_or_else(|_| {
            panic!(
                "could not find joint_minimax_N12.json (tried {grid_path} and relative variants)"
            )
        });
    ...
    let contribs: Result<Vec<f64>, ferric_core::FerricError> = grids
        .omega_points
        .par_iter()
    ...
    println!(
        "Minimax AO-RPA Energy: {:.12} Hartree (Time: {:.2?})",
        e_corr_ao, duration_ao
    );
    println!(
        "Exact MO-RPA Energy:   {:.12} Hartree (Time: {:.2?})",
        e_corr_mo, duration_mo
    );
    println!("--------------------------------------------------");
    println!("Quadrature Error (abs):  {:+.6e} Hartree", diff);
    println!(
        "Quadrature Error (rel):  {:.6e} ({:.4}%)",
        rel_diff,
        rel_diff * 100.0
    );
    println!("==================================================");
