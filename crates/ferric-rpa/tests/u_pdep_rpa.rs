...
#[test]
#[ignore = "H atom edge case (nocc_β = 0) — ferric E_c ≠ PySCF U-RI-RPA reference (-0.0096 vs -0.0134). \
            OH (both spins occupied) matches to 1e-4 Ha. Tracked as #30 — likely a subtle \
            factor in the only-one-spin-occupied limit; needs Π_σ-by-eigenvalue comparison \
            vs PySCF to localize."]
fn u_pdep_rpa_h_atom_matches_pyscf() {
    ...
    // PySCF: E_scf = -0.4992784034, E_c = -0.0134148241.
    let ctx = ParallelContext::default();
    let xyz = "1\nh\nH 0 0 0\n";
    ...
    // RPA correlation: -0.0134148241. Allow 1e-5 for quadrature + Davidson noise.
    assert!((r.e_rpa - (-0.0134148241)).abs() < 1e-5,
            "U-RPA E_c disagree: ferric {}, pyscf -0.01341", r.e_rpa);
}
