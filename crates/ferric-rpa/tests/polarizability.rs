...
#[test]
fn nh3_esp_matches_pyscf() {
    // NH3 / cc-pVDZ reference generated by scripts/gen_pyscf_rpa_props.py
    // (testdata/reference/nh3_cc-pvdz_rpa_props.json). Second molecule beyond
    // H2 widening the "H2O only" ESP-at-nuclei caveat in docs/VALIDATION.md.
    let r = load_ref("../../testdata/reference/nh3_cc-pvdz_rpa_props.json");
    let (mol, obs, _dfbs, _op, rhf) = setup_nh3_ccpvdz();
    let v = esp_at_atoms(&mol, &obs, rhf.density_r()).unwrap();
    assert_eq!(v.len(), r.esp_at_atoms.len());
    for (a, (got, exp)) in v.iter().zip(r.esp_at_atoms.iter()).enumerate() {
        let d = (got - exp).abs();
