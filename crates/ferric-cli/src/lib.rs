...
fn run_mp3(
    ...
    let mp3_result = mp3_energy(mol, prep, &dfbs, op, result, cfg.mp2.frozen_core.resolve(mol), budget_bytes)
        .unwrap_or_else(|e| {
            eprintln!("error: {e}");
            std::process::exit(1);
        });
...
fn run_mp2_v(
    ...
    let att_cfg = cfg.mp2.build_att_vv10_config(mol, budget_bytes).unwrap_or_else(|e| {
        eprintln!("error: {e}");
        std::process::exit(1);
    });
...
fn run_gw(
    ...
    budget_bytes: Option<usize>,
        ...
        let gw_frozen_core = cfg.gw.frozen_core.unwrap_or(cfg.rpa.frozen_core).resolve(mol);
        let rpa_cfg = PdepRpaConfig {
...
fn run_bse_tda(
    ...
    budget_bytes: Option<usize>,
        ...
        let bse_frozen_core = cfg.gw.frozen_core.unwrap_or(cfg.rpa.frozen_core).resolve(mol);
        let rpa_cfg = PdepRpaConfig {
...
fn run_tdhf_static_polarizability(
    ...
    budget_bytes: Option<usize>,
        ...
        let frozen_core = cfg.gw.frozen_core.unwrap_or(cfg.rpa.frozen_core).resolve(mol);
