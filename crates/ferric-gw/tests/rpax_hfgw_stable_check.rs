...
fn hf_gw_diagonal_is_stable_water() {
    ...
    assert!(
        res.alpha_static > 0.0,
        "HF+GW-diagonal path must give POSITIVE static alpha (sanity re-confirmation)"
    );
    ...
    let n = nocc * nvir;
    let (v_dressed, _dev) = w_pdep::redress_with_check(
        &mo_b::build_full_b(&mol, &obs, &dfbs, op, &rhf, 0, None)
            .unwrap()
            .v_inv_sqrt,
        &gw.pdep.eigenpotentials,
    )
    .unwrap();
    ...
    let screened = |p: usize, q: usize, r: usize, s: usize| -> f64 {
        ...
        for al in 0..m_modes {
            acc += w_red[al] * m_proj[(al, p, q)] * m_proj[(al, r, s)];
        }
        acc
    };
    ...
    assert!(
        min_amb > 0.0,
        "with the real GW-corrected diagonal, (A-B) should be positive-definite for water"
    );
}
