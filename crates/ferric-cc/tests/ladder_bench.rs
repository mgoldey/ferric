...
use ferric_cc::helpers::{contract_hh_ladder, contract_pp_ladder};
...
/// RI complexity: O(N^5)
pub fn contract_pp_ladder_scalar(b_ab: &Array3<f64>, t2: &Array4<f64>) -> Array4<f64> {
...
/// RI complexity: O(N^5)
pub fn contract_hh_ladder_scalar(b_ij: &Array3<f64>, t2: &Array4<f64>) -> Array4<f64> {
...
fn bench(naux: usize, nocc: usize, nvir: usize) {
    ...
    let b_ab =
        Array3::from_shape_vec((naux, nvir, nvir), fill(&mut seed, naux * nvir * nvir)).unwrap();
    let b_ij =
        Array3::from_shape_vec((naux, nocc, nocc), fill(&mut seed, naux * nocc * nocc)).unwrap();
    let t2 = Array4::from_shape_vec(
        (nocc, nvir, nocc, nvir),
        fill(&mut seed, nocc * nvir * nocc * nvir),
    )
    .unwrap();
    ...
    let max_hh = hh_e
        ...
        .fold(0.0f64, f64::max);
    assert!(
        max_pp < 1e-9 && max_hh < 1e-9,
        "einsum != scalar: pp {max_pp:.2e} hh {max_hh:.2e}"
    );

    let reps = 5;
    let t = Instant::now();
    for _ in 0..reps {
        let _ = contract_pp_ladder_scalar(&b_ab, &t2);
        let _ = contract_hh_ladder_scalar(&b_ij, &t2);
    }
    let t_scalar = t.elapsed().as_secs_f64() / reps as f64;

    let t = Instant::now();
    for _ in 0..reps {
        let _ = contract_pp_ladder(&b_ab_t, &t2_t);
        let _ = contract_hh_ladder(&b_ij_t, &t2_t);
    }
